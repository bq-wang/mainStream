...
///////////////////////////////////////////////////////////////////////////////
// in this chapter, the knowledge of the unicode will be discussed	     //
//   there are a couple new types and new operations (mostly are conversion_ //
// added to the c++0x							     //
///////////////////////////////////////////////////////////////////////////////

// below is the mbstate_t static field to be used
// outside of the CoeConvert function
std::mbstate_t out_cvt_state;

template
<typename In, typename Out, typename InS, typename OutS, size_t Max_In_Size  = 4>
...
// apparently , the codecvt for the u16string is not yet ready to be used.
// and if you call the following code, it will throw out the exception
//  std::u16string s3 = u"This is a u16string literal";
//  std::string converted = CodeConvertU16(s3);
//  it will throw the bad cast exception
std::string 
CodeConvertU16(const std::u16string & s )
{
	 ...
	 char * argv[]) 
{
  ...
  auto u8_s = u8"This is a u8 string"; // 
  const char * charu8_s = u8"This is au8 string literal";

  std::cout << typeid(u8_s).name()
	    << std::endl;

  std::cout << "typeid(char16_s)" 
	    << "is "
	    << typeid(char16_s).name()
	    << std::endl;


  // there are added convertert that 
  // aim to help convert
...
//  Behaves like std::codecvt<char16_t, char, std::mbstate_t>	   //
  ...
  std::string out_str = CodeConvert<wchar_t, char, std::wstring, std::string, 4>(s2);
  std::cout << "wstring: "
  	    << out_str
  	    << std::endl;

  // std::string 			
  // out_str = CodeConvert<char16_t, char, std::u16string, std::string, 4>(s3);
  // std::cout << "u16string: "
  // 	    << out_str
  // 	    << std::endl;

  // std::string 			
  // out_str = CodeConvert<char32_t, char, std::u32string, std::string, 4>(s4);
  // std::cout << "u32string: "
  // 	    << out_str
  // 	    << std::endl;
 ...
 // out_str = CodeConvertU16(s3);
  ...
  // 	    << out_str
  // 	    << std::endl;
  // the following code will demonstrate the use of raw string
  std::string noNewLines (R"(\n\n)");
  std::string cmd(R"(ls /home/docs | grep ".pdf")");

  // the raw string can host some newlines.
  std::string withNewLines(R"(Line 1 of the string...
Line2...
Line3)");
  // and you can even declare raw string for any encodings
  std::wstring u16rawString = LR"(Raw Wide String literal \t (without a tab))";
  std::string rawU8String = u8R"(Raw UTF-8 string literal \n(without a blank))";
  std::u16string rawu16String = uR"(Raw UTF-16 string literal \\ (with two blackslashs)";
  std::u32string rawu32String = UR"(Raw UTF-32 string literal \u2620 (without a code point)";
